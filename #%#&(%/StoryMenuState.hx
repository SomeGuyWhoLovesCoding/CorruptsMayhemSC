...
class StoryMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (curCode == 'EMPTY' && FlxG.save.data.emptyUnlocked)
		{
			PlayState.SONG = Song.loadFromJson('empty', 'empty');
			FlxG.sound.play(Paths.sound('teleport-empty'), 16);
			black.alpha = 1;
			new FlxTimer().start(0.11499999, function(tmr:FlxTimer)
			{
					LoadingState.loadAndSwitchState(new PlayState()); 
			});
		}

		if (codeInt == 5 && FlxG.save.data.emptyUnlocked)
		{
				curCode = 'EMPTY';
				codeInt = 0;
		}*/
		...
		scoreText.text = "WEEK SCORE:" + lerpScore/* + " (" + codeInt + ")"*/;
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if(sprDifficulty.graphic != newImage)
		{
			...
			sprDifficulty.y = leftArrow.y + 12;

			if(tweenDifficulty != null) tweenDifficulty.cancel();
			tweenDifficulty = FlxTween.tween(sprDifficulty, {y: leftArrow.y + 4, alpha: 1}, 0.1357, {ease: FlxEase.expoOut, onComplete: function(twn:FlxTween)
			{
